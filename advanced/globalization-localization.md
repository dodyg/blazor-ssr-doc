...
Blazor provides built-in support to render values in the current culture. Therefore, specifying a culture with [`@bind:culture`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bindculture) isn't recommended when using the `date` and `number` field types.
...
In the preceding example, [`CultureTypes.SpecificCultures`](https://learn.microsoft.com/dotnet/api/system.globalization.culturetypes) returns only cultures that are specific to a country or region—such as `en-US` or `fr-FR`—which come with full, concrete globalization data (for dates, numbers, calendars, and other cultural UI) that .NET can use for accurate formatting and parsing. Neutral cultures, such as `en` or `fr`, may not have complete globalization data, so they aren't included in this list.
...
Prevent Blazor autostart by adding `autostart="false"` to [Blazor's `<script>` tag](https://learn.microsoft.com/aspnet/core/blazor/project-structure#location-of-the-blazor-script):

```html
<script src="{BLAZOR SCRIPT}" autostart="false"></script>
```

**In the preceding example, the `{BLAZOR SCRIPT}` placeholder is the Blazor script path and file name.** For the location of the script, see [project-structure](https://learn.microsoft.com/aspnet/core/blazor/project-structure#location-of-the-blazor-script).

Add the following `<script>` block after [Blazor's `<script>` tag](https://learn.microsoft.com/aspnet/core/blazor/project-structure#location-of-the-blazor-script) and before the closing `</body>` tag:
...
Provide JS functions after [Blazor's `<script>` tag](https://learn.microsoft.com/aspnet/core/blazor/project-structure#location-of-the-blazor-script) to get and set the user's culture selection with browser local storage:
...
For information on ordering the Localization Middleware in the middleware pipeline, see [index](https://learn.microsoft.com/aspnet/core/fundamentals/middleware/index#middleware-order).
...
After the [Blazor's `<script>` tag](https://learn.microsoft.com/aspnet/core/blazor/project-structure#location-of-the-blazor-script), provide JS functions to get and set the user's culture selection with browser local storage:
...
Use [Localization Middleware](https://learn.microsoft.com/aspnet/core/fundamentals/localization#localization-middleware) to set the app's culture.
...
If the app should localize resources based on storing a user's culture setting, use a localization culture cookie. Use of a cookie ensures that the WebSocket connection can correctly propagate the culture. If localization schemes are based on the URL path or query string, the scheme might not be able to work with [WebSockets](https://learn.microsoft.com/aspnet/core/fundamentals/websockets), thus fail to persist the culture. Therefore, the recommended approach is to use a localization culture cookie. See the [Dynamically set the server-side culture by user preference](#dynamically-set-the-server-side-culture-by-user-preference) section of this article to see an example Razor expression that persists the user's culture selection.
...
For additional guidance, see [localization](https://learn.microsoft.com/aspnet/core/fundamentals/localization).
