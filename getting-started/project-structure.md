...
More information on components and render modes is found in the [index](/components/index) and [render-modes](/fundamentals/render-modes) articles.
...
The `Components/Pages` folder of the server project contains the app's routable server-side Razor components. The route for each page is specified using the [`@page`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#page) directive.


:::moniker range=">= aspnetcore-10.0"

The `NotFound` component (`NotFound.razor`) implements a Not Found page to display when content isn't found for a request path. For more information, see [navigation](https://learn.microsoft.com/aspnet/core/blazor/fundamentals/navigation#not-found-responses).
...
An `_Imports.razor` file in each of the server and `.Client` projects includes common Razor directives for Razor components of either project, such as [`@using`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#using) directives for namespaces.
...
* The `Pages` folder contains routable client-side Razor components. The route for each page is specified using the [`@page`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#page) directive.

* The `wwwroot` folder is the [Web Root](https://learn.microsoft.com/aspnet/core/fundamentals/index#web-root) folder for the `.Client` project that holds the app's public static assets.

* The `Program.cs` file is the project's entry point that sets up the WebAssembly [host](https://learn.microsoft.com/aspnet/core/fundamentals/host/generic-host#host-definition) and contains the project's startup logic, including service registrations, configuration, logging, and request processing pipeline.
...
* `Pages` folder: Contains the Blazor app's routable Razor components (`.razor`) and the root Razor page of a Blazor Server app. The route for each page is specified using the [`@page`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#page) directive. The template includes the following:
...
* `Program.cs`: The app's entry point that sets up the ASP.NET Core [host](https://learn.microsoft.com/aspnet/core/fundamentals/host/generic-host).
...
* `Pages` folder: Contains the Blazor app's routable Razor components (`.razor`). The route for each page is specified using the [`@page`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#page) directive. The template includes the following components:
  * `Counter` component (`Counter.razor`): Implements the Counter page.
  * `Index` component (`Index.razor`): Implements the Home page.
  * `Weather` component (`Weather.razor`): Implements the Weather page.
  * `NotFound` component (`NotFound.razor`): Implements a Not Found page to display when content isn't found for a request path. For more information, see [navigation](https://learn.microsoft.com/aspnet/core/blazor/fundamentals/navigation#not-found-responses).
...
The solution is generated from the Blazor WebAssembly project template in Visual Studio with the **ASP.NET Core Hosted** checkbox selected or with the `-ho|--hosted` option using the .NET CLI's `dotnet new blazorwasm` command. For more information, see [tooling](https://learn.microsoft.com/aspnet/core/blazor/tooling).
...
The Blazor script is served as a static web asset with automatic compression and fingerprinting. For more information, see [static-files](https://learn.microsoft.com/aspnet/core/blazor/fundamentals/static-files).
...
* [tooling](https://learn.microsoft.com/aspnet/core/blazor/tooling)
* [hosting-models](/getting-started/comparison)
* [apis](https://learn.microsoft.com/aspnet/core/fundamentals/apis)
