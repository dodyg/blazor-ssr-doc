...
The following `BindGetSet` component demonstrates `:get`/`:set` syntax for `<input>` elements and the [`InputText` component](xref:blazor/forms/input-components) used by [Blazor forms](/forms/index) in synchronous (`Set`) and asynchronous (`SetAsync`) scenarios.
...
* Use an [input component](xref:blazor/forms/input-components), such as <xref:Microsoft.AspNetCore.Components.Forms.InputNumber%601> or <xref:Microsoft.AspNetCore.Components.Forms.InputDate%601>, with [form validation](/forms/validation). Input components together with form validation components provide built-in support to manage invalid inputs:
