...
Razor components provide data binding features with the [`@bind`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bind) Razor directive attribute with a field, property, or Razor expression value.
...
When the `BindTheory` component is rendered, the `value` of the HTML demonstration `<input>` element comes from the `InputValue` property. When the user enters a value in the text box and changes element focus, the `onchange` event is fired and the `InputValue` property is set to the changed value. In reality, code execution is more complex because [`@bind`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bind) handles cases where type conversions are performed. In general, [`@bind`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bind) associates the current value of an expression with the `value` attribute of the `<input>` and handles changes using the registered handler.
...
* [Delegate event handlers](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#delegate-event-handlers)
* [Custom event arguments](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#custom-event-arguments)



To bind on other DOM events, use [delegate event handlers](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#delegate-event-handlers).
...
Using an [event callback parameter (`EventCallback`/`EventCallback<T>`)](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#eventcallback) with `@bind:after` isn't supported. Instead, pass a method that returns an [System.Action](https://learn.microsoft.com/dotnet/api/system.action) or [System.Threading.Tasks.Task](https://learn.microsoft.com/dotnet/api/system.threading.tasks.task) to `@bind:after`.
...
For more information on the `InputText` component, see [input-components](https://learn.microsoft.com/aspnet/core/blazor/forms/input-components).
...
The following `BindGetSet` component demonstrates `:get`/`:set` syntax for `<input>` elements and the [`InputText` component](https://learn.microsoft.com/aspnet/core/blazor/forms/input-components) used by [Blazor forms](/forms/index) in synchronous (`Set`) and asynchronous (`SetAsync`) scenarios.
...
> [!NOTE]
> Across multiple components, two-way binding to a property with `get`/`set` accessors requires discarding the [System.Threading.Tasks.Task](https://learn.microsoft.com/dotnet/api/system.threading.tasks.task) returned by [Microsoft.AspNetCore.Components.EventCallback.InvokeAsync *?displayProperty=nameWithType](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback.invokeasync%2a?displayproperty=namewithtype) in the property's setter. For two-way data binding, we recommend using `@bind:get`/`@bind:set` modifiers. For more information, see the [`@bind:get`/`@bind:set` guidance](#binding-features) earlier in this article.
>
> To see an example of how the [System.Threading.Tasks.Task](https://learn.microsoft.com/dotnet/api/system.threading.tasks.task) returned by [Microsoft.AspNetCore.Components.EventCallback.InvokeAsync *?displayProperty=nameWithType](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback.invokeasync%2a?displayproperty=namewithtype) is discarded in .NET 6 or earlier before `@bind:get`/`@bind:set` modifiers became a framework feature, see [the `NestedChild` component of the *Bind across more than two components* section in the .NET 6 version of this article](?view=aspnetcore-6.0&preserve-view=true#bind-across-more-than-two-components).



> [!NOTE]
> Two-way binding to a property with `get`/`set` accessors requires discarding the [System.Threading.Tasks.Task](https://learn.microsoft.com/dotnet/api/system.threading.tasks.task) returned by [Microsoft.AspNetCore.Components.EventCallback.InvokeAsync *?displayProperty=nameWithType](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback.invokeasync%2a?displayproperty=namewithtype). For an example, see [the `NestedChild` component of the *Bind across more than two components* section](#bind-across-more-than-two-components). For two-way data binding in .NET 7 or later, we recommend using `@bind:get`/`@bind:set` modifiers, which are described in .NET 7 or later versions of this article.



## Multiple option selection with `<select>` elements

Binding supports [`multiple`](https://developer.mozilla.org/docs/Web/HTML/Attributes/multiple) option selection with `<select>` elements. The [`@onchange`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#onevent) event provides an array of the selected elements via [event arguments (`ChangeEventArgs`)](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#event-arguments). The value must be bound to an array type.
...
* Use an [input component](https://learn.microsoft.com/aspnet/core/blazor/forms/input-components), such as [Microsoft.AspNetCore.Components.Forms.InputNumber`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.forms.inputnumber%601) or [Microsoft.AspNetCore.Components.Forms.InputDate`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.forms.inputdate%601), with [form validation](/forms/validation). Input components together with form validation components provide built-in support to manage invalid inputs:
  * Permit the user to provide invalid input and receive validation errors on the associated [Microsoft.AspNetCore.Components.Forms.EditContext](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.forms.editcontext).
  * Display validation errors in the UI without interfering with the user entering additional webform data.

## Format strings

Data binding works with a single [System.DateTime](https://learn.microsoft.com/dotnet/api/system.datetime) format string using `@bind:format="{FORMAT STRING}"`, where the `{FORMAT STRING}` placeholder is the format string. Other format expressions, such as currency or number formats, aren't available at this time but might be added in a future release.
...
Nullable [System.DateTime?displayProperty=fullName](https://learn.microsoft.com/dotnet/api/system.datetime?displayproperty=fullname) and [System.DateTimeOffset?displayProperty=fullName](https://learn.microsoft.com/dotnet/api/system.datetimeoffset?displayproperty=fullname) are supported:
...
You can't implement chained binds with [`@bind`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bind) syntax in a child component. An event handler and value must be specified separately to support updating the property in the parent from the child component. The parent component still leverages [`@bind`](https://learn.microsoft.com/aspnet/core/mvc/views/razor#bind) syntax to set up data binding with the child component.

The following `ChildBind` component has a `Year` component parameter and an [Microsoft.AspNetCore.Components.EventCallback`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback%601). By convention, the [Microsoft.AspNetCore.Components.EventCallback`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback%601) for the parameter must be named as the component parameter name with a "`Changed`" suffix. The naming syntax is `{PARAMETER NAME}Changed`, where the `{PARAMETER NAME}` placeholder is the parameter name. In the following example, the [Microsoft.AspNetCore.Components.EventCallback`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback%601) is named `YearChanged`.

[Microsoft.AspNetCore.Components.EventCallback.InvokeAsync *?displayProperty=nameWithType](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback.invokeasync%2a?displayproperty=namewithtype) invokes the delegate associated with the binding with the provided argument and dispatches an event notification for the changed property.
...
For more information on events and [Microsoft.AspNetCore.Components.EventCallback`1](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback%601), see the *EventCallback* section of the [event-handling](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#eventcallback) article.
...
* Exposes changes of a `Password` property to a parent component with an [`EventCallback`](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#eventcallback) that passes in the current value of the child's `password` field as its argument.
* Uses the `onclick` event to trigger the `ToggleShowPassword` method. For more information, see [event-handling](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling).
...
Prior to the release of .NET 7, two-way binding across components uses `get`/`set` accessors with a third property that discards the [System.Threading.Tasks.Task](https://learn.microsoft.com/dotnet/api/system.threading.tasks.task) returned by [Microsoft.AspNetCore.Components.EventCallback.InvokeAsync *?displayProperty=nameWithType](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.eventcallback.invokeasync%2a?displayproperty=namewithtype) in its setter. To see an example of this approach for .NET 6 or earlier before `@bind:get`/`@bind:set` modifiers became a framework feature, see [the `NestedChild` component of this section in the .NET 6 version of this article](?view=aspnetcore-6.0&preserve-view=true#bind-across-more-than-two-components).
...
For an alternative approach suited to sharing data in memory and across components that aren't necessarily nested, see [index](https://learn.microsoft.com/aspnet/core/blazor/state-management/index).
...
The following `ChildParameterExpression` component identifies the `Year` expression's model and field name. A [Microsoft.AspNetCore.Components.Forms.FieldIdentifier](https://learn.microsoft.com/dotnet/api/microsoft.aspnetcore.components.forms.fieldidentifier), which is used to obtain the model and field name, uniquely identifies a single field that can be edited. This may correspond to a property on a model object or can be any other named value. Use of a parameter's expression is useful when creating custom validation components, which isn't covered by the Microsoft Blazor documentation but is addressed by numerous third-party resources.
...
* [Parameter change detection and additional guidance on Razor component rendering](https://learn.microsoft.com/aspnet/core/blazor/components/rendering)
* [index](/forms/index)
* [Binding to radio buttons in a form](https://learn.microsoft.com/aspnet/core/blazor/forms/binding#radio-buttons)
* [Binding `InputSelect` options to C# object `null` values](https://learn.microsoft.com/aspnet/core/blazor/forms/binding#binding-inputselect-options-to-c-object-null-values)
* [ASP.NET Core Blazor event handling: `EventCallback` section](https://learn.microsoft.com/aspnet/core/blazor/components/event-handling#eventcallback)
* [Blazor samples GitHub repository (`dotnet/blazor-samples`)](https://github.com/dotnet/blazor-samples) ([how to download](https://learn.microsoft.com/aspnet/core/blazor/fundamentals/index#sample-apps))
